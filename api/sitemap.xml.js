import { supabaseGetPaged } from "./_lib/supabase.js";
...
export default async function handler(req, res) {
  // PostgRESTのプロジェクト側デフォルト行数上限（1000件）はクエリ文字列のlimit=Nでは
  // 超えられない（本番で実際に3クエリとも1000件で打ち切られることを確認済み）ため、
  // Rangeヘッダーによるページングで取得する（supabaseGetPaged、詳細はCLAUDE.md参照）。
  const broadcasters = await supabaseGetPaged(`tracked_broadcasters?select=broadcaster_name&order=last_seen_at.desc`, 5000);
  const clippers = await supabaseGetPaged(`top_clippers_mv?select=creator_id&order=total_views.desc`, 3000);
  const clips = await supabaseGetPaged(`clips?id=neq.__general_thread__&select=id,twitch_created_at&order=view_count.desc`, 5000);
