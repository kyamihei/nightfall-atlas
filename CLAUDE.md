...
- **絞り込み行（期間・並び替え・ゲーム）の右側の余白**: 左寄せ化（直前の項目）の結果、
  行の右側に何もない帯状の余白ができて気になるという指摘。`rankingControlsRow`に
  `weeklyBoard`（週間クリップ職人ランキング）と同じカード風の背景・枠線・角丸を付け、
  左寄せの配置自体は維持しつつ、余白が「コントロール行という1つの箱の一部」に見えるように
  した（ページ生地がむき出しに見える状態を解消）。
