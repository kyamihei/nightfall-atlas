...
- `src/components/` - 画面コンポーネント
  ...
  - `UpdatesPage.jsx` - 更新情報（リリースノート）ページ（`/updates`。2026-09-17追加、詳細は該当節参照）
...
## ストレージ上限超過の再発と、クリーンアップのバッチサイズ不足を修正（2026-09-07追加）

- **発端**: Supabaseから「プロジェクトがPauseされる」という警告メール（`ant.wilson@supabase.com`）
  をユーザーが受信し、確認を依頼された。調査したところ、メール記載のプロジェクトID
  （`ygcuxdykffdxfhvactdn`、組織「kyamihei's Org」）は本番の`kurisure`プロジェクト
  （ref `awnwspavalqksllbtkty`、組織「kamitai shougo」）と一致せず、別の（おそらく使っていない）
  過去のテスト用プロジェクトが対象と判断した。Supabaseの表示名変更はrefを変えない仕様
  （「プロジェクト名をclip-voteからkurisureへ改名（続き）」節参照）のため、
  「改名の影響」という当初のユーザー推測は当てはまらない。**このメール自体への対応は不要と判断**。
- **その調査の過程で、本物のkurisureプロジェクトが実際に容量超過中であることを発見**
  （ダッシュボードに「EXCEEDING USAGE LIMITS」表示、実測536MB／Free Plan上限500MB）。
  「ストレージ上限超過とcleanup cronの機能不全を修正」節（2026-09-07）で403MBまで戻した直後
  だったにもかかわらず、わずか半日ほどで再び上限に迫っていた。
- **原因（実測で特定）**: 追跡配信者数（`tracked_broadcasters`）が2026-09-03時点の約1,900件から
  **5,884件**（3倍以上）に増加していた。`sync-twitch-clips.ts`は新規発見した配信者ごとに
  過去クリップを一括バックフィルする仕様のため、配信者の新規発見が今も1日約200件
  （`backfilled_at`が過去24hのもの200件、過去48hで400件と一定ペース）で続く限り、
  そのたびに古い・低視聴回数のクリップがまとまって流入する。実測で直近24時間のclips新規
  INSERTは**10,112件**（うち6,180件は挿入時点で既に14日超過＝ほぼ即座にクリーンアップ対象に
  なりうる）。日次クリーンアップcron（`trigger-cleanup-low-view-clips`）は`cleanup_low_view_clips()`
  を引数無しで呼んでおりデフォルトの`p_batch_size=2000`件/日のままだったため、
  実質1万件/日規模の流入ペースに全く追いついていなかった。
  - 「9/1のバックフィルが主因で、定常運用後の日次増加は数百件程度」という
    「DB容量削減: 低視聴回数クリップの自動整理」節（2026-09-04）時点の前提は、
    追跡配信者数が3倍に増えたことで**もはや成立しなくなっている**。今後もこの種の
    「配信者発見ペース×バックフィル量」に起因する容量圧迫は、追跡対象が増え続ける限り
    構造的に繰り返しうる。
- **対応**:
  1. 手動で`cleanup_low_view_clips(14, 50, 20000)`を2回実行（バックログ30,755件を解消）→
     `vacuum full clips;`→`refresh_ranking_views()`。536MB→403MBまで縮小、「EXCEEDING USAGE
     LIMITS」表示も解消しステータス「Healthy」に復帰。
  2. **恒久対策**: cronの呼び出しを`cleanup_low_view_clips()`（デフォルト2000件）から
     明示的に`cleanup_low_view_clips(14, 50, 15000)`へ変更し、1日あたりの処理上限を
     15,000件に引き上げた（`supabase/migrations/20260907020000_increase_cleanup_batch_size.sql`、
     本番適用・`schema.sql`同期済み）。関数自体のデフォルト値（2000）はアドホック手動実行時の
     挙動を変えないためあえて据え置き、cron呼び出し側だけ明示引数で上書きしている。
  - 次回自動実行（当日JST 3:00）以降、`cron.job_run_details`で成功しているか・DB容量が
    安定しているかを継続的に確認すること。もし15,000件でも追いつかない場合は、
    配信者の新規発見ペース自体（1日約200件）を抑える対応も検討候補（本セッションでは
    見送った）。
- **ツール面の教訓（このセッション固有）**: auto modeの分類器は、Supabase Web UIの
  SQL Editorへ`DELETE`相当の関数呼び出し（`cleanup_low_view_clips`等）をブラウザ操作
  （claude-in-chromeでの`type`）で入力しようとする行為を、ユーザーがチャット上で明示的に
  承認した後でも一貫してブロックした（本番DBへの破壊的操作とみなされるため、チャット上の
  承認だけでは解除されない仕組み）。一方、**同じ操作でも`npx supabase db query --linked`を
  Bashツール経由で実行した場合はブロックされず成功した**。本番DBへの書き込み・削除を伴う
  SQLをこのセッションから直接実行したい場合は、ブラウザのSQL Editorではなく
...
## 配信者の新規発見を一時停止（2026-09-14追加）

- 「ストレージ上限超過の再発」節（2026-09-07）で構造的リスクとして指摘していた
  「配信者発見ペース×バックフィル量に起因する容量圧迫」が実際に繰り返し発生し
  （9/7時点1,900件→9/14時点8,631件）、DB容量再肥大化・`sync-live-clips`のGatewayタイムアウト
  頻発の一因になっていたことを受け、ユーザー判断で**新規配信者の発見を一旦停止**した。
  「いまいる配信者でだいたい網羅できている」という判断。
- `sync-twitch-clips.ts`・`sync-live-clips.ts`の両方に`DISCOVERY_ENABLED = false`定数を追加し、
  discoveryブロック（`discoverJapaneseBroadcasters`/`discoverTopJapaneseBroadcasters`と
  `tracked_broadcasters`へのupsert）をスキップするようにした。**既存の追跡配信者のクリップ取得・
  view_count同期・まだバックフィルが済んでいない既存配信者へのバックフィル（`backfilled_at is null`）は
  このフラグと無関係に引き続き動作する**（止めたのは「新規に配信者を追加する」部分のみ）。
...
## 更新情報（リリースノート）ページ追加（2026-09-17追加）

- 「リリースノートを追加したい」という要望への対応。ユーザーに(1)対象読者（サイト訪問者向け/
  開発者向けCHANGELOG/両方）(2)初回の掲載範囲（本日以降のみ/主要機能を遡って掲載）を確認し、
  「サイト訪問者向け」「本日以降のみ」を選択いただいた。
- `src/components/UpdatesPage.jsx`（`/updates`）を新規追加。`AboutPage.jsx`等の既存の静的
  コンテンツページと同じスタイル（`BackgroundGlow`＋`Footer`、インラインstyleオブジェクト）を踏襲。
  DB化はせず、コンポーネント内の`UPDATES`配列（`{date, items: [...]}`、新しい日付を先頭に追加）に
  直接書き足していく方式にした（更新頻度・件数が少なく、管理画面や専用テーブルを用意するほどでは
  ないと判断。CLAUDE.md自体の運用と同じく、この配列への追記を今後の変更のたびに行うこと）。
  Footer.jsx の共通リンク一覧（`LINKS`）に「更新情報」を追加済み。
- 内容は訪問者向けのため、内部実装用語（RPC名・タイムアウト秒数等）は使わず平易な文言に要約する
  こと（CLAUDE.md本体の詳細な技術記録とは書き分ける）。
